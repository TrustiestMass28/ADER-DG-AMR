...
template<int P>
class AmrDG : public Solver<AmrDG<P>>, public std::enable_shared_from_this<AmrDG<P>>
{
  public:
    //type alias to improve readibility
    using NumericalMethodType = AmrDG<P>;

    // Bring dependent base class members into scope for two-phase lookup
    using Solver<AmrDG<P>>::mesh;
    using Solver<AmrDG<P>>::Q;
    using Solver<AmrDG<P>>::Q_unique;
    using Solver<AmrDG<P>>::T;
    using Solver<AmrDG<P>>::Dt;
    using Solver<AmrDG<P>>::CFL;
    using Solver<AmrDG<P>>::c_dt;
    using Solver<AmrDG<P>>::flag_source_term;
    using Solver<AmrDG<P>>::U_w;
    using Solver<AmrDG<P>>::U;
    using Solver<AmrDG<P>>::U_center;
    using Solver<AmrDG<P>>::S;
    using Solver<AmrDG<P>>::F;
    using Solver<AmrDG<P>>::Fm;
    using Solver<AmrDG<P>>::Fp;
    using Solver<AmrDG<P>>::DFm;
    using Solver<AmrDG<P>>::DFp;
    using Solver<AmrDG<P>>::Fnum;
    using Solver<AmrDG<P>>::Fnum_int_f;
    using Solver<AmrDG<P>>::Fnum_int_c;
    using Solver<AmrDG<P>>::cf_face_b_coarse;
    using Solver<AmrDG<P>>::cf_face_b_fine;
    using Solver<AmrDG<P>>::cf_face_child_idx;
    using Solver<AmrDG<P>>::n_pt_bc;
    using Solver<AmrDG<P>>::get_null_BC;
    using Solver<AmrDG<P>>::get_null_BC_vct;
    using Solver<AmrDG<P>>::PlotFile;
    using Solver<AmrDG<P>>::FillBoundary;
    using Solver<AmrDG<P>>::FillBoundaryCells;
    using Solver<AmrDG<P>>::ofs;
    using Solver<AmrDG<P>>::m_tag_impl;
    using Solver<AmrDG<P>>::flag_analytical_ic;
    using Solver<AmrDG<P>>::dtn_outplt;
    using Solver<AmrDG<P>>::dt_outplt;
    using Solver<AmrDG<P>>::data_dir;
    using Solver<AmrDG<P>>::restart_tstep;
    using Solver<AmrDG<P>>::restart_time;
    using Solver<AmrDG<P>>::m_bar;
    using std::enable_shared_from_this<AmrDG<P>>::shared_from_this;

    AmrDG()  = default;

    ~AmrDG();

    void settings(amrex::Real _T, amrex::Real _c_dt,
    ...
    class L2ProjInterp : public Solver<AmrDG<P>>::template AMR_Interpolation<L2ProjInterp>
    {
      public:
        // Bring dependent base class member into scope for two-phase lookup
        using Solver<AmrDG<P>>::template AMR_Interpolation<L2ProjInterp>::numme;
  ...
  private:
    ...
    // Compile-time basis sizes
    static constexpr int Np_s  = BasisLegendre<P>::Np_s;
    static constexpr int Np_st = BasisLegendre<P>::Np_st;

    // Runtime basis index tables (populated from BasisLegendre<P> at init)
    ...
    QuadratureGaussLegendre<P> quadrule;
...
template<int P>
template <typename EquationType>
void AmrDG<P>::set_initial_condition(const std::shared_ptr<ModelEquation<EquationType>>& model_pde, int lev)
{
...
template<int P>
template <typename EquationType>
amrex::Real AmrDG<P>::set_initial_condition_U_w(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,int lev,int q,int n,int i,int j,int k)
{
...
template<int P>
template <typename EquationType>
amrex::Real AmrDG<P>::set_initial_condition_U(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                                            int lev,int q,int i,int j,int k,
                                            const amrex::Vector<amrex::Real>& xi) const
{
...
template<int P>
template <typename EquationType>
void AmrDG<P>::evolve(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
...
template<int P>
template <typename EquationType>
void AmrDG<P>::time_integration(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                            const std::shared_ptr<BoundaryCondition<EquationType,NumericalMethodType>>& bdcond,
                            amrex::Real time)
{
  auto _mesh = mesh.lock();
...
template<int P>
template <typename EquationType>
void AmrDG<P>::ADER(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                 const std::shared_ptr<BoundaryCondition<EquationType,NumericalMethodType>>& bdcond,
                  amrex::Real time)
{ 
...
template<int P>
template <typename EquationType>
void AmrDG<P>::flux(int lev, int d, int M,
                 const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                 amrex::MultiFab* _U,
                 amrex::MultiFab* _F,
                 const amrex::Vector<amrex::Vector<amrex::Real>>& xi)
...
template<int P>
template <typename EquationType>
void AmrDG<P>::flux_bd(int lev,int d, int M,
                    const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                    amrex::MultiFab* _U,
                    amrex::MultiFab* _F,
                    amrex::MultiFab* _DF,
...
template<int P>
template <typename EquationType>
void AmrDG<P>::set_Dt(const std::shared_ptr<ModelEquation<EquationType>>& model_pde)
{
...
template<int P>
template <typename EquationType>
void AmrDG<P>::source(int lev,int M,
                    const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                    amrex::MultiFab* _U,
                    amrex::MultiFab* _S,
                    const amrex::Vector<amrex::Vector<amrex::Real>>& xi)
...
template<int P>
template <typename EquationType>
void AmrDG<P>::LpNorm_DG_AMR(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                                  int _p, amrex::Vector<amrex::Vector<amrex::Real>> quad_pt, int N) const
{
  auto _mesh = mesh.lock();
...
template<int P>
template <typename EquationType>
void AmrDG<P>::L1Norm_DG_AMR(const std::shared_ptr<ModelEquation<EquationType>>& model_pde)
{
...
template<int P>
template <typename EquationType>
void AmrDG<P>::L2Norm_DG_AMR(const std::shared_ptr<ModelEquation<EquationType>>& model_pde) 
{ 
...
template<int P>
template <typename EquationType>
int AmrDG<P>::Limiter_w(const std::shared_ptr<ModelEquation<EquationType>>& model_pde, int lev)
{
  auto _mesh = mesh.lock();
...
template<int P>
template <typename EquationType>
bool AmrDG<P>::Limiter_linear_tvb(const std::shared_ptr<ModelEquation<EquationType>>& model_pde,
                              int i, int j, int k,
                              amrex::Vector<amrex::Array4<amrex::Real>>* uw,
                              amrex::Vector<amrex::Array4<amrex::Real>>* vw,
                              int lev)
...
// Template implementation files — included here so the compiler sees definitions
// for whatever P is instantiated in the translation unit that includes this header.
#include "AmrDG_Basis.tpp"
#include "AmrDG_Quadrature.tpp"
#include "AmrDG.tpp"
#include "AmrDG_ElementMatrix.tpp"
#include "AmrDG_NumericalFlux.tpp"
#include "AmrDG_TagCellRef.tpp"
#include "AmrDG_Limiter.tpp"
#include "AmrDG_Level.tpp"
#include "AmrDG_Interpolater.tpp"
#include "AmrDG_Utils.tpp"
